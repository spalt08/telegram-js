...
function makeSearchSession(peer: Peer): SearchSession {
  ...
  const result = new BehaviorSubject<SearchResult>({ request: desiredRequest, ids: [], isFull: true });
  ...
  function search(request: SearchRequest) {
    ...
    if (isRequestEmpty(request)) {
      ...
      result.next({ request, ids: [], isFull: true });
