...
const headerArray = strToArray(header);
const footer = [0xff, 0xd9];

export function strippedToBlob(stripped: ArrayBuffer): Blob {
  const bytes = new Uint8Array(stripped);
  headerArray[164] = bytes[1];
  headerArray[166] = bytes[2];
  const buffer = new Uint8Array(headerArray.length + bytes.length);
  buffer.set(headerArray);
  buffer.set(bytes.slice(3), headerArray.length);
  buffer.set(footer, headerArray.length + bytes.length - 3);
  return arrayToBlob(buffer, 'image/jpeg');
}
...
export function getThumbnail(sizes: PhotoSize[]) {
  for (let i = 0; i < sizes.length; i += 1) {
    ...
    if (size._ === 'photoCachedSize') {
      const blob = arrayToBlob(size.bytes, 'image/jpeg');
      return blobToUrl(blob);
    }
  }
